...
// 2] Else use the $FLOBOT_TEMPDIR environment variable
...
class TempService {
  ...
  boot (options, callback) {
    ...
    const fromEnvVariable = process.env.FLOBOT_TEMPDIR
    const fromSystem = os.tmpdir()

    options.messages.detail('Config: ' + fromConfig)
    options.messages.detail('$FLOBOT_TEMPDIR: ' + fromEnvVariable)
    options.messages.detail('System: ' + fromSystem)

    /**
     * This is the value as derived by first considering if a `tempDir` value is specified in the config, failing that if a `$FLOBOT_TEMPDIR` environment variable has been set... and if all else fails use the system temp dir (i.e. the value returned by `os.tmpdir()`)
     * @property {string} tempDir The root of Flobot's temporary directory
     * @example
  ...
  makeTempDir (subDirPath, callback) {
    const fullPath = path.join(this.tempDir, 'flobot', subDirPath)
    mkdirp(
      fullPath,
      function (err) {
        if (err) {
