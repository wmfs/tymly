...
function packageObjCallback (gitHubPackageObj) {
  ...
  if (publicOnly && !isPublicPackage(gitHubPackageObj)) return
...
function shouldSync (gitHubPackageObj) {
  if (gitHubPackageObj.config && gitHubPackageObj.config.tymly) { return gitHubPackageObj.config.tymly.sync !== false }
  return true
}

function isPublicPackage (gitHubPackageObj) {
  return gitHubPackageObj.publishConfig
    ? gitHubPackageObj.publishConfig.access === 'public'
    : false
}
