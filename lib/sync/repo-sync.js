...
module.exports = async function repoSync (options) {
  ...
  for (const repo of sorted) {
    ...
    if (repo.repoType !== currentType) {
      console.log('')
      console.log(`    ${chalk.cyan(`/${repo.repoType}${repo.repoType === 'cardscript' ? '' : 's'}`)}`)
      process.stdout.write('      ')
      currentType = repo.repoType
    }
    const creds = `${options.gitHubUser}:${options.gitHubToken}@github.com`

    const [success, action] = await cloneOrPull(repo, creds, newRepos, gitErrs, unstagedChangesErr)
...
async function cloneOrPull (repo, creds, newRepos, gitErrs, unstagedChangesErr) {
  const cloneUrl = repo.meta.clone_url
  const destPath = path.resolve(__dirname, '../..', repo.repoType + `${repo.repoType === 'cardscript' ? '' : 's'}`, repo.repoName)

  try {
    const [success, action] = await cloneRepo(cloneUrl, creds, destPath)
    if (success) {
      newRepos.push(`${repo.repoName} ${repo.repoType}`)
