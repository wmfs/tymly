...
const UNMERGED_CHANGES = /merge is not possible/
...
module.exports = async function repoSync (options) {
  ...
  for (const repo of sorted) {
    ...
    const [success, action] = await cloneOrPull(repo, creds, cloneErrs, unstagedChangesErr)
...
async function cloneOrPull (repo, creds, cloneErrs, unstagedChangesErr) {
  ...
  try {
    ...
    return pullRepo(repo.repoName, destPath, unstagedChangesErr)
  } catch (err) {
    cloneErrs.push(`${repo.repoName} : ${err.message}`)
    return [false, 'cloned']
  }
}
...
async function pullRepo (repoName, destPath, unstagedChangesErr) {
  try {
