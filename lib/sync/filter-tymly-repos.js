...
module.exports = async function filterTymlyRepos (options) {
  logger.section('Filtering Tymly repos:')
  const applyRepoData = async () => {
    const tymlyRepos = []
    await Promise.all(
      options.gitHubRepos.map(async repo => {
        try {
  ...
  Object.getOwnPropertyNames(stats).forEach(repoType => {
    ...
    logger.success(`Found ${chalk.bold(stat.total)} Tymly ${repoType}s ${stat.private > 0 ? `(${stat.private} of which are private)` : ''}`)
    allTotal += stat.total
