...
function packageDetails (dir, absoluteDir, exclusions, logger) {
  logger(`Bundling ${dir} ...`)
  const packages = readVersionNumbers(absoluteDir, gatherPackages(dir, exclusions))
...
async function bundleForDeploy (dir, workDir, exclusions = [], bundleName = 'bundle', logger = () => {}) {
  const absoluteDir = path.resolve(dir)
  const packages = packageDetails(dir, absoluteDir, exclusions, logger)
